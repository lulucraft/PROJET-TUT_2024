...
export class DataService {
  ...
  // public static sizes: Category[] = [];
  // async sizes(): Promise<Category[]> {
  ...
  removeProductFromCart(productId: number): void {
    this.cart = this.cart.filter(cp => cp.product.id !== productId);
    localStorage.setItem('cart', JSON.stringify(this.cart));
  }

  setProductQuantity(productId: number, quantity: number): void {
    if (quantity > 0) {
      let productAlreadyInCart: CartProduct | undefined = this.cart.find(cp => cp.product.id === productId);
      if (productAlreadyInCart) {
  ...
  getSizes(): Observable<Category[]> {
    return this.http.get<Category[]>(this.apiBaseUrl + 'api/user/categories');
  }
